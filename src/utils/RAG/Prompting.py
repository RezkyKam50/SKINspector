...
class MessageBuilder:
    ...
    def build_rag_messages(self, image_data_uri: str, unique_docs: List[Document]) -> List[Dict[str, Any]]:
        try:
                ...
                try:
                    ...
                    if content and len(content) > 50: 
                        ...
                        if len(cleaned_content) > 100:  
                            # Remove the length restriction or increase it significantly
                            context_parts.append(cleaned_content)  # Removed [:800] limit
            ...
            if context_parts:
                # REMOVE THE [:3] LIMIT - this is what's causing only 3 paragraphs
                context_text = "\n\n".join(context_parts)  # Changed from [:3] to no limit
    ...
    def _get_fallback_messages(self, image_data_uri: str) -> List[Dict[str, Any]]:
        system_message = {
            "role": "system",
            "content": (
                "You are an expert dermatologist. Since no reference materials are available, "
